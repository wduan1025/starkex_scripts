...
def getMintableErc721AssetType(address):
   ...
   print(Web3.solidityKeccak(["bytes", "uint256"], [asset_info, 1]))
   asset_type = Web3.solidityKeccak(asset_info, 1) & 0x03FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF
   return asset_type


def getMintableErc721AssetId(minting_blob, address):
   blob_hash = Web3.solidityKeccak(["string"], [minting_blob])
   asset_type = getMintableErc721AssetType(address)
   hash = Web3.solidityKeccak(["string", "uint256", "bytes"], ['MINTABLE:', asset_type, blob_hash])
...
print('mintable erc721 asset id is', getMintableErc721AssetId(minting_blob, contract_address))
